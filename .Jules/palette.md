...
## 2026-03-30 - Progress Indicator for Multi-step flows
**Learning:** Multi-step flows like GenerateImage need clear structural ARIA definitions because visually grouping child elements (like colored dots) doesn't naturally communicate sequence or scale to screen readers.
**Action:** Use `role="progressbar"`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` on the parent container, and `aria-current="step"` on the active child to communicate progress comprehensively.
