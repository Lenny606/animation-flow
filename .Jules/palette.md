...
## 2024-07-15 - ARIA Roles for Multi-step components
**Learning:** Adding a visual progress bar significantly improves usability for multi-step flows, but it must be made accessible to screen readers.
**Action:** Multi-step components in the frontend should implement visual progress indicators equipped with structural ARIA attributes (`role="progressbar"`, `aria-valuenow`, `aria-valuemin`, `aria-valuemax` on the container, and `aria-current="step"` on the active child) to clearly convey the current step and total steps to screen readers.
