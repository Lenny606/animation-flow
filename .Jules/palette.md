...
## 2024-10-24 - ARIA Progress Bars for Multi-Step Flows
**Learning:** Multi-step components without progress indicators leave users uncertain about the process length and their current position.
**Action:** When implementing or improving multi-step components, add visual progress indicators equipped with structural ARIA attributes (`role="progressbar"`, `aria-valuenow`, `aria-valuemin`, `aria-valuemax` on the container, and `aria-current="step"` on the active step element) to clearly convey the current step and total steps to screen readers.