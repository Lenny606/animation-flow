...
## 2024-10-26 - ARIA Progressbars for Multi-Step Components
**Learning:** Multi-step components in the frontend rely on conditional rendering to show different steps. While visual progress indicators are helpful, they lack context for screen readers if not properly structured.
**Action:** When implementing visual progress indicators for multi-step flows, always equip them with structural ARIA attributes: `role="progressbar"`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` on the container, and `aria-current="step"` on the active child element to clearly convey the current step and total steps to screen readers.
